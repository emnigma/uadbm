...
class GMVAE_spatial(AEMODEL):
    class Config(AEMODEL.Config):
        def __init__(self):
            super().__init__('GMVAE_spatial')
            self.dim_c = 6
            self.dim_z = 1
            self.dim_w = 1
            self.c_lambda = 1
    ...
    def train(self, dataset):
        ...
        self.z_mu = tf.tile(tf.expand_dims(self.z_mu, -1), [1, 1, 1, 1, self.dim_c])
        z_logvar = tf.tile(tf.expand_dims(self.z_log_sigma, -1), [1, 1, 1, 1, self.dim_c])
        ...
        con_prior_loss = tf.reduce_sum(tf.squeeze(tf.matmul(kl, tf.expand_dims(self.pc, -1)), -1), [1, 2, 3])
        ...
        w_loss = 0.5 * tf.reduce_sum(tf.square(self.w_mu) + tf.exp(self.w_log_sigma) - self.w_log_sigma - 1, [1, 2, 3])
        self.losses['w_prior_loss'] = mean_w_loss = tf.reduce_mean(w_loss)

        # 4. KL(q(c|z)||p(c)) =  - sum_k q(k) log p(k)/q(k) , k = dim_c
        # let p(k) = 1/K#
        closs1 = tf.reduce_sum(tf.multiply(self.pc, tf.log(self.pc * self.dim_c + 1e-8)), [3])
